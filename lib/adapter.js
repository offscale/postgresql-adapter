...
const Adapter = {
  ...
  registerConnection (connection, collections, cb) {
    ...
    if (Adapter.datastores.get(connection.identity)) {
    ...
    return Util.initializeConnection(cxn)
      .then(() => {
        Adapter.datastores.set(connection.identity, cxn);
        cb()
      })
      .catch(cb)
  },
  ...
  describe (connectionName, tableName, cb) {
    const cxn = Adapter.datastores.get(connectionName);
  ...
  teardown (conn, cb = conn) {
    const datastores = conn ? [Adapter.datastores.get(conn)] : Adapter.datastores.values();
    const teardownPromises = [];

    for (const cxn of datastores) {
      if (!cxn) continue;
    ...
    return Promise.all(teardownPromises)
      .then(() => {
        ...
        for (const cxn of datastores) {
          if (!cxn) continue;
          Adapter.datastores.delete(cxn.identity)
        }
        cb()
      })
      .catch(cb)
   ...
   */
  knex (connectionName) {
    const cnx = Adapter.datastores.get(connectionName);
    if (cnx) {
      return cnx.knex
    }
  }
